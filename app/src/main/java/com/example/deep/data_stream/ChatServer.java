...
/**
 * A Tiny Http Server extended from {@link NanoHTTPD}. Once started, serves selected files from {@link ChatActivity} on an assigned PORT.
 * <p>
 * Created by Sri on 18/12/16.
 */

class ChatServer extends NanoHTTPD {
    ...
    public ChatServer(String host_name, int port) {
        super(host_name, port);
    }

    public ChatServer(Context context, FileTransferStatusListener statusListener, String[] filesToBeHosted) {
        this(null, 0);
        m_context = context;
    ...
    public ChatServer(Context context, FileTransferStatusListener statusListener, String[] filesToBeHosted, int port) {
        this(null, port);
        m_context = context;
        m_clientsFileTransferListener = statusListener;
        m_filesTobeHosted = filesToBeHosted;
    ...
    /**
     * Creates a success <code>Response</code> with binary data of file in {@link ChatServer#m_filesTobeHosted} with provided index.
     *
