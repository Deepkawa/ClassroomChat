 ...
 */
package com.example.deep.data_stream;
...
import com.example.deep.utils.DividerItemDecoration;
import com.example.deep.utils.RecyclerViewArrayAdapter;
import com.example.deep.utils.Utils;
import com.example.deep.utils.HotspotControl;
import com.example.deep.utils.WifiUtils;
...
import static com.example.deep.data_stream.ChatActivity.ShareUIHandler.LIST_API_CLIENTS;
import static com.example.deep.data_stream.ChatActivity.ShareUIHandler.UPDATE_AP_STATUS;
import static com.example.deep.utils.Utils.getRandomColor;
...
public class ChatActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (null != getIntent() && getIntent().hasExtra(ChatService.EXTRA_FILE_PATHS))
            m_sharedFilePaths = getIntent().getStringArrayExtra(ChatService.EXTRA_FILE_PATHS);
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    ...
                    if (Build.VERSION.SDK_INT >= 23 &&
                            ...
                            Utils.getTargetSDKVersion(getApplicationContext()) >= 23 && !Settings.System.canWrite(ChatActivity.this)) {
        ...
        m_p2pServerUpdatesListener = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (isFinishing() || null == intent)
                    return;
                int intentType = intent.getIntExtra(ChatService.ShareIntents.TYPE, 0);
                if (intentType == ChatService.ShareIntents.Types.FILE_TRANSFER_STATUS) {
                    String fileName = intent.getStringExtra(ChatService.ShareIntents.SHARE_SERVER_UPDATE_FILE_NAME);
                    updateReceiverListItem(intent.getStringExtra(ChatService.ShareIntents.SHARE_CLIENT_IP), intent.getIntExtra(ChatService.ShareIntents.SHARE_TRANSFER_PROGRESS, -1), intent.getStringExtra(ChatService.ShareIntents.SHARE_SERVER_UPDATE_TEXT), fileName);
                } else if (intentType == ChatService.ShareIntents.Types.AP_DISABLED_ACKNOWLEDGEMENT) {
                    shouldAutoConnect = false;
                    resetSenderUi(false);
                }
            }
        };
        registerReceiver(m_p2pServerUpdatesListener, new IntentFilter(ChatService.ShareIntents.SHARE_SERVER_UPDATES_INTENT_ACTION));
    }
    ...
    private void disableAp() {
        ...
        Intent p2pServiceIntent = new Intent(getApplicationContext(), ChatService.class);
        p2pServiceIntent.setAction(ChatService.WIFI_AP_ACTION_STOP);
        startService(p2pServiceIntent);
        isApEnabled = false;
    }

    /**
     * Starts {@link ChatService} with intent action {@link ChatService#WIFI_AP_ACTION_START} to enableShareThemHotspot Hotspot and start {@link ChatServer}.
     */
    private void startP2pSenderWatchService() {
        Intent p2pServiceIntent = new Intent(getApplicationContext(), ChatService.class);
        p2pServiceIntent.putExtra(ChatService.EXTRA_FILE_PATHS, m_sharedFilePaths);
        if (null != getIntent()) {
            p2pServiceIntent.putExtra(ChatService.EXTRA_PORT, getIntent().getIntExtra(ChatService.EXTRA_PORT, 0));
            p2pServiceIntent.putExtra(ChatService.EXTRA_SENDER_NAME, getIntent().getStringExtra(ChatService.EXTRA_SENDER_NAME));
        }
        p2pServiceIntent.setAction(ChatService.WIFI_AP_ACTION_START);
        startService(p2pServiceIntent);
    }

    /**
     * Starts {@link ChatService} with intent action {@link ChatService#WIFI_AP_ACTION_START_CHECK} to make {@link ChatService} constantly check for Hotspot status. (Sometimes Hotspot tend to stop if stayed idle for long enough. So this check makes sure {@link ChatService} is only alive if Hostspot is enaled.)
     */
    private void startHostspotCheckOnService() {
        Intent p2pServiceIntent = new Intent(getApplicationContext(), ChatService.class);
        p2pServiceIntent.setAction(ChatService.WIFI_AP_ACTION_START_CHECK);
        startService(p2pServiceIntent);
    }

    /**
     * Calls methods - {@link ChatActivity#updateApStatus()} & {@link ChatActivity#listApClients()} which are responsible for displaying Hotpot information and Listing connected clients to the same
     */
     ...
     * Constantly adds/updates receiver items on {@link ChatActivity#m_receiversList}
    ...
    /**
     * Adds a {@link HotspotControl.WifiScanResult} item to {@link ChatActivity#m_receiversListAdapter} if not already added
     *
...
//            m_receiversListAdapter.remove(new WifiApControl.Client(ip, null, null));
    ...
    static class ShareUIHandler extends Handler {
        ...
        ShareUIHandler(ChatActivity activity) {
